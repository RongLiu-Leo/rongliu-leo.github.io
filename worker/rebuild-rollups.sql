...
DELETE FROM places;
DELETE FROM page_places;
DELETE FROM daily;
DELETE FROM page_daily;
DELETE FROM hourly;
DELETE FROM page_hourly;
DELETE FROM referrers;
DELETE FROM page_referrers;
...
INSERT INTO page_places (page, lat, lon, country, region, city, n)
SELECT page, lat, lon, COALESCE(country, ''), COALESCE(region, ''), COALESCE(city, ''), COUNT(*)
  FROM views
 WHERE lat IS NOT NULL AND lon IS NOT NULL
 GROUP BY page, lat, lon, COALESCE(country, ''), COALESCE(region, ''), COALESCE(city, '');
...
INSERT INTO page_hourly (page, hour, n)
SELECT page, CAST(strftime('%H', created_at / 1000, 'unixepoch') AS INTEGER), COUNT(*)
  FROM views
 GROUP BY 1, 2;
...
INSERT INTO page_referrers (page, host, n)
SELECT page, referrer, COUNT(*)
  FROM views
 WHERE referrer IS NOT NULL AND referrer <> ''
